...
if __name__ == '__main__':
    ...
    parser.add_argument('--last_hidden', '-lh', type=int, default=0)
    ...
    for layer_num in n_layers:
        for d_r in dropout_rate:
            ...
            if args.last_hidden == 0:
                lex_net = LexNET(path_encoder, class_n=n_classes, n_w_vocab=n_lemma, w_emb_size=50,
                                 embed_initial=lemma_embed, dropout=d_r)
            else:
                lex_net = LexNET_h(path_encoder, class_n=n_classes, n_w_vocab=n_lemma, w_emb_size=50,
                                   embed_initial=lemma_embed, dropout=d_r)

            model = Classifier(lex_net)
            if args.gpu >= 0:
                chainer.cuda.get_device_from_id(args.gpu).use()
                model.to_gpu()
            ...
            while c_val <= 7:
                ...
                for i in range(0, n_train, args.batchsize):
                    ...
                    loss = model(c_w1s, c_w2s, c_paths, c_labels)
                ...
                current_val_score = lex_net.evaluate(val_w1s, val_w2s, val_paths, val_labels)
                ...
                if current_val_score > max_val_score:
                    ...
                    serializers.save_npz(args.out + '/best.lex_net', lex_net)
                    test_score = lex_net.evaluate(test_w1s, test_w2s, test_paths, test_labels)
